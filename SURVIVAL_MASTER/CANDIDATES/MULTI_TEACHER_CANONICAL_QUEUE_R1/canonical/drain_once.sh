...
STATE_ROOT="$ARTIFACT_ROOT/MULTI_TEACHER_CANONICAL_QUEUE_R1_FIX1"
...
# Deterministic content-addressed serialization only. This is not semantic ranking.
...
for READY in "${READY_FILES[@]}"; do
  ...
  [ "$(field_file SCHEMA "$DESC")" = SIGMA_MULTI_TEACHER_WEIGHT_CANDIDATE_R2 ] || hold PACKET_SCHEMA_MISMATCH
  [ "$(field_file CANDIDATE_ID "$DESC")" = "$CID" ] || hold PACKET_ID_MISMATCH
  ...
  EVID="$PACKET/teaching.evidence"
  NSRC="$PACKET/native_learning.sigma"
  NBC="$PACKET/native_learning.sigmab"
  RUNNER="$PACKET/mechanical_replay_runner"
  OWN="$PACKET/native_ownership.receipt"
  ...
  for P in "$EVID" "$NSRC" "$NBC" "$RUNNER" "$OWN"; do [ -f "$P" ] || hold PACKET_BOUND_FILE_MISSING; done
  ...
  [ "$(field_file SCHEMA "$RESULT")" = SIGMA_MULTI_TEACHER_CANONICAL_RESULT_R2 ] || hold RESULT_SCHEMA_MISMATCH
  ...
  [ "$(field_file NATIVE_LEARNING_SOURCE_SHA256 "$RESULT")" = "$NSRC_SHA" ] || hold RESULT_SOURCE_HASH_MISMATCH
  [ "$(field_file NATIVE_LEARNING_BYTECODE_SHA256 "$RESULT")" = "$NBC_SHA" ] || hold RESULT_BYTECODE_HASH_MISMATCH

  NATIVE_DECISION="$(field_file NATIVE_DECISION_RECEIPT_PATH "$RESULT")"
  NATIVE_DECISION_SHA="$(field_file NATIVE_DECISION_RECEIPT_SHA256 "$RESULT")"
  [ -n "$NATIVE_DECISION" ] || hold NATIVE_DECISION_RECEIPT_PATH_MISSING
  NATIVE_DECISION="$(realpath "$NATIVE_DECISION")"
  under "$NATIVE_DECISION" "$ATTEMPT" || hold NATIVE_DECISION_RECEIPT_OUTSIDE_ATTEMPT
  [ -f "$NATIVE_DECISION" ] || hold NATIVE_DECISION_RECEIPT_MISSING
  [ "$(sha "$NATIVE_DECISION")" = "$NATIVE_DECISION_SHA" ] || hold NATIVE_DECISION_RECEIPT_HASH_MISMATCH
  [ "$(field_file SCHEMA "$NATIVE_DECISION")" = SIGMA_NATIVE_CANONICAL_LEARNING_DECISION_R1 ] || hold NATIVE_DECISION_SCHEMA_MISMATCH
  [ "$(field_file SOURCE_CANDIDATE_ID "$NATIVE_DECISION")" = "$CID" ] || hold NATIVE_DECISION_CANDIDATE_MISMATCH
  [ "$(field_file PARENT_HEAD "$NATIVE_DECISION")" = "$BEFORE_HEAD" ] || hold NATIVE_DECISION_PARENT_HEAD_MISMATCH
  [ "$(field_file PARENT_MODEL_GENERATION "$NATIVE_DECISION")" = "$BEFORE_GEN" ] || hold NATIVE_DECISION_PARENT_GENERATION_MISMATCH
  [ "$(field_file NATIVE_LEARNING_SOURCE_SHA256 "$NATIVE_DECISION")" = "$NSRC_SHA" ] || hold NATIVE_DECISION_SOURCE_HASH_MISMATCH
  [ "$(field_file NATIVE_LEARNING_BYTECODE_SHA256 "$NATIVE_DECISION")" = "$NBC_SHA" ] || hold NATIVE_DECISION_BYTECODE_HASH_MISMATCH
  [ "$(field_file SIGMA_NATIVE_LEARNING_OWNER "$NATIVE_DECISION")" = YES ] || hold NATIVE_DECISION_OWNER_INVALID

  QRESULT="$(field_file QUEUE_RESULT "$RESULT")"
  NATIVE_QRESULT="$(field_file QUEUE_RESULT "$NATIVE_DECISION")"
  [ "$QRESULT" = "$NATIVE_QRESULT" ] || hold HOST_RESULT_DIFFERS_FROM_NATIVE_DECISION
  ...
  case "$QRESULT" in
    ACCEPTED)
      ...
      {
        echo 'QUEUE_PROCESSED=ACCEPTED_BY_NATIVE_SIGMA'
        ...
        echo "NATIVE_DECISION_RECEIPT_SHA256=$NATIVE_DECISION_SHA"
      ...
      printf 'QUEUE_DRAIN=ACCEPTED_BY_NATIVE_SIGMA\nCANDIDATE_ID=%s\nAFTER_MODEL_GENERATION=%s\n' "$CID" "$AFTER_GEN"
      exit 0
      ;;
    REJECTED)
    ...
    HOLD)
      ...
      printf 'QUEUE_DRAIN=HOLD_FROM_NATIVE_SIGMA\nCANDIDATE_ID=%s\nATTEMPT=%s\n' "$CID" "$ATTEMPT"
      exit 2
      ;;
