...
CANDIDATE_VALUE_ABSENT_FROM_SIGMA_SOURCE="NOT_APPLICABLE"
if [ "${TOP_COUNT:-0}" = "1" ] && [ -n "${SOURCE_PATH:-}" ] && [ -n "${EVIDENCE_LINE:-}" ]; then
    ...
    if [ -n "${VALUE:-}" ] && ! grep -Fq -- "$VALUE" "$SRC"; then
        ...
        CANDIDATE_VALUE_ABSENT_FROM_SIGMA_SOURCE="FAIL"
    fi
fi

{
    ...
    echo "CANDIDATE_VALUE_ABSENT_FROM_SIGMA_SOURCE=$CANDIDATE_VALUE_ABSENT_FROM_SIGMA_SOURCE"
...
if [ "$CANDIDATE_VALUE_ABSENT_FROM_SIGMA_SOURCE" = "FAIL" ]; then
    echo "OPEN_SEMANTIC_V0_1=HOLD_CANDIDATE_VALUE_FOUND_IN_SOURCE"
    exit 11
fi

if [ "${STATUS:-}" = "EVIDENCE_BOUND_SINGLE_CANDIDATE" ] && [ "$EVIDENCE_BINDING" = "PASS" ] && [ "$CANDIDATE_VALUE_ABSENT_FROM_SIGMA_SOURCE" = "PASS" ]; then
