...
printf 'SIGMA_PHASE=V24_1_STEP_LIMIT_PREFLIGHT\n'
...
run_case() {
    ...
    INPUT_PARTIAL="$T_INPUT.partial.$$"
    "$P/bin/rm" -f -- "$INPUT_PARTIAL" || return 72
    "$P/bin/cp" -- "$DOC" "$INPUT_PARTIAL" || return 72
    "$P/bin/chmod" 0400 "$INPUT_PARTIAL" || return 72
    "$P/bin/mv" -f -- "$INPUT_PARTIAL" "$T_INPUT" || return 72
