...
STATE="$HOME_SIGMA/SIGMA_V4B2_REAL_V24_RC9_REPLAY_PREFLIGHT"
SHADOW="$STATE/shadow"
...
if ! "$P/bin/flock" -n 9; then
    printf 'HOLD=V4B2_PREFLIGHT_ALREADY_RUNNING\n'
    exit 20
fi
...
printf 'SIGMA_PHASE=V4B2_REAL_V24_RC9_HELD_CONTEXT_REPLAY_PREFLIGHT\n'
...
RC=$?
printf 'V4B2_SIGMAC_RC=%s\n' "$RC"
[ "$RC" -eq 0 ] || { printf 'HOLD=V4B2_COMPILE_FAILED\n'; exit 27; }
[ -s "$BC.partial" ] || { printf 'HOLD=V4B2_BYTECODE_EMPTY\n'; exit 28; }
mv -f -- "$BC.partial" "$BC"
chmod 0400 "$BC"
printf 'V4B2_BYTECODE_SHA256=%s\n' "$(hash1 "$BC")"
...
run_fixed_replay() {
    ...
    grep -F -x "CTX=$SHA || COMPLETE=YES" "$COMPLETE" >/dev/null || { printf 'HOLD=REAL_CONTEXT_NOT_COMPLETED context=%s status=%s\n' "$SHA" "$FINAL_STATUS"; return 49; }

    grep -F 'STATUS REFUSE_' "$LOG/$SHA."*.log >/dev/null 2>&1 && { printf 'HOLD=REAL_CONTEXT_NATIVE_REFUSAL context=%s\n' "$SHA"; return 50; }

    [ "$FINAL_STATUS" = 'ALREADY_COMPLETE' ] || [ "$FINAL_STATUS" = 'CONTEXT_COMPLETE' ] || [ "$FINAL_STATUS" = 'RECOVERED_COMPLETION' ] || {
...
printf '\nV4B2_REAL_V24_RC9_HELD_CONTEXT_REPLAY_PREFLIGHT=PASS\n'
...
printf 'NEXT_ACTION=CHECKPOINT_V4B2_THEN_INTEGRATE_V4A_ARBITER_WITH_V4B_SEGMENTED_LEARNER_IN_SHADOW_CONTINUOUS_CONTROLLER\n'
