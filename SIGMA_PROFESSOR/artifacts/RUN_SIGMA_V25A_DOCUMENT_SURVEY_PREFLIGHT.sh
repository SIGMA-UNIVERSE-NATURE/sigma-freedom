...
SRC="$E/SIGMA_DOCUMENT_SURVEY_V2_5A.sigma"
BC="$E/SIGMA_DOCUMENT_SURVEY_V2_5A.sigmab"
EXPECTED_SOURCE=8848c5fd96fdd16508462a3a58cc2c803af96461c59049c77b45aece6fd239e8
...
printf 'SIGMA_PHASE=V25A_DOCUMENT_SURVEY_PREFLIGHT\n'
...
if [ "$PASS" -eq 1 ] && [ "$SURVEYED_COUNT" -eq 3 ] && [ "$RECORD_COUNT" -eq 3 ] && [ "$COMPLETE_SEEN" -eq 1 ]; then
    printf 'V25A_DOCUMENT_SURVEY_PREFLIGHT=PASS\n'
    printf 'NEXT_ACTION=BUILD_V25_FULL_CORPUS_SURVEY_RUNNER\n'
    exit 0
fi

printf 'V25A_DOCUMENT_SURVEY_PREFLIGHT=FAIL\n'
