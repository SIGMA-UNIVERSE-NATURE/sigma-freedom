...
PATCHED="$ROOT/RUN_SIGMA_LANG_01G_PREFLIGHT_R3_FRESH_STATE_INIT.sh"
...
printf 'SIGMA_PHASE=LANG_01G_FRESH_STATE_INITIALIZATION_REPAIR_R3\n'
printf 'REPAIR_CLASS=RUNNER_ONLY_MECHANICAL_FRESH_STATE_FIXTURE_INITIALIZATION\n'
...
printf 'FRESH_STATE_REPRESENTATION=ZERO_LENGTH_STATE_FILE\n'
...
mkdir_count=$("$P/bin/grep" -F -x -c '    "$P/bin/mkdir" -p "$IN" "$STATE_DIR"' "$BASE_RUNNER" || true)
[ "$mkdir_count" -eq 1 ] || {
    printf 'HOLD=LANG_01G_PREPARE_CASE_MKDIR_OCCURRENCE_NOT_EXACTLY_ONE\n'
    exit 33
}

"$P/bin/awk" \
    -v old_pin="$OLD_RECORDED_SOURCE" \
    -v new_pin="$EXPECTED_SOURCE" '
{
    if ($0 == "EXPECTED_SOURCE=" old_pin) {
        print "EXPECTED_SOURCE=" new_pin
        next
    }

    print $0

    if ($0 == "    \"$P/bin/mkdir\" -p \"$IN\" \"$STATE_DIR\"") {
        print "    : > \"$STATE_FILE\""
    }
}
' "$BASE_RUNNER" > "$PATCHED"

new_pin_count=$("$P/bin/grep" -F -x -c "EXPECTED_SOURCE=$EXPECTED_SOURCE" "$PATCHED" || true)
old_after=$("$P/bin/grep" -F -x -c "EXPECTED_SOURCE=$OLD_RECORDED_SOURCE" "$PATCHED" || true)
fresh_state_init_count=$("$P/bin/grep" -F -x -c '    : > "$STATE_FILE"' "$PATCHED" || true)
...
[ "$fresh_state_init_count" -eq 1 ] || {
    printf 'HOLD=LANG_01G_FRESH_STATE_INIT_OCCURRENCE_NOT_EXACTLY_ONE\n'
    exit 36
}
...
printf 'FRESH_STATE_INITIALIZATION_REPAIR=PASS\n'
