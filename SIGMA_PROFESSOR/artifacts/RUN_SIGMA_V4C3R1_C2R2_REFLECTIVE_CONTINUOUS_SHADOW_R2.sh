...
LOG="$STATE/log_reflective_c3r1_r2"
...
VM_SHA=$(hash1 "$VM")
printf 'SIGMA_V4C3R1_C2R2_REFLECTIVE_CONTINUOUS_SHADOW_R2=START\n'
...
A3_REPO="$REPO/$A3_REL"; B4_REPO="$REPO/$B4_REL"; C2_REPO="$REPO/$C2_REL"; C3_REPO="$REPO/$C3_REL"
...
A3_BLOB=$(blob1 "$A3_REPO"); B4_BLOB=$(blob1 "$B4_REPO"); C2_BLOB=$(blob1 "$C2_REPO"); C3_BLOB=$(blob1 "$C3_REPO")
A3_SHA=$(hash1 "$A3_REPO"); B4_SHA=$(hash1 "$B4_REPO"); C2_SHA=$(hash1 "$C2_REPO"); C3_SHA=$(hash1 "$C3_REPO")
printf 'A3_GIT_BLOB=%s\nB4_GIT_BLOB=%s\nC2_GIT_BLOB=%s\nC3_GIT_BLOB=%s\n' "$A3_BLOB" "$B4_BLOB" "$C2_BLOB" "$C3_BLOB"
printf 'A3_SOURCE_SHA256=%s\nB4_SOURCE_SHA256=%s\nC2_SOURCE_SHA256=%s\nC3_SOURCE_SHA256=%s\n' "$A3_SHA" "$B4_SHA" "$C2_SHA" "$C3_SHA"
...
install_compile() {
    ...
    INSTALLED_BLOB=$(git -C "$REPO" hash-object "$SRC"); INSTALLED_SHA=$(hash1 "$SRC")
...
BUDGET_LEN=$(wc -c < "$C3_BUDGET" | tr -d ' '); PAUSE_LEN=$(wc -c < "$C3_PAUSE" | tr -d ' ')
[ "$BUDGET_LEN" -eq "$REFLECTION_PROGRESS_BUDGET" ] || { printf 'HOLD=C3_PROGRESS_BUDGET_STATE_MISMATCH\n'; exit 44; }
[ "$PAUSE_LEN" -eq "$OBSERVE_PAUSE_SECONDS" ] || { printf 'HOLD=C3_PAUSE_STATE_MISMATCH\n'; exit 45; }
...
transport_exact_native_line_request() {
    ...
    REQ_ID=$(cat "$READ_REQ_ID"); DOC_ID=$(cat "$READ_DOC"); LINE_U=$(cat "$READ_LINE"); PURPOSE=$(cat "$READ_PURPOSE")
    printf 'CORPUS_TRANSPORT TURN=%s REQUEST_ID=%s DOC=%s LINE_UNARY_LEN=%s PURPOSE=%s\n' "$TURN" "$REQ_ID" "$DOC_ID" "${#LINE_U}" "$PURPOSE"
    [ -n "$REQ_ID" ] || return 80; [ "$EXPECTED_TARGET" = "$REQ_ID" ] || return 81; [ -n "$DOC_ID" ] || return 82
    case "$DOC_ID" in *'/'*|*'..'*) return 83 ;; esac
    case "$PURPOSE" in PROFILE|LEARN) ;; *) return 84 ;; esac
    REST=${LINE_U//|/}; [ -z "$REST" ] || return 85
    DOC_PATH="$RAW/$DOC_ID.document"; [ -f "$DOC_PATH" ] || return 86
    LINE_NO=$(( ${#LINE_U} + 1 )); FOUND_MARK=$("$P/bin/sed" -n "${LINE_NO}{=;q;}" "$DOC_PATH")
    TMP_RESULT="$STATE/.line-result.$TURN.$$"; rm -f -- "$TMP_RESULT"
    if [ "$FOUND_MARK" = "$LINE_NO" ]; then
        LINE_VALUE=$("$P/bin/sed" -n "${LINE_NO}p" "$DOC_PATH"); printf '%s' "$LINE_VALUE" > "$TMP_RESULT" || return 87; printf '%s' YES > "$READ_RESULT_FOUND" || return 88
    else
        : > "$TMP_RESULT"; printf '%s' NO > "$READ_RESULT_FOUND" || return 89
    fi
...
run_c3_observable() {
    ...
    wait "$C3_PID"; C3_RC=$?
    "$P/bin/cat" "$C3LOG"
    [ "$C3_RC" -eq 0 ] || { printf 'HOLD=C3_VM_FAILURE TURN=%s RC=%s LOG=%s\n' "$TURN" "$C3_RC" "$C3LOG"; return 1; }
    if "$P/bin/grep" -F 'V4C3R1_STATUS REFUSE_' "$C3LOG" >/dev/null 2>&1; then printf 'HOLD=C3_NATIVE_REFLECTION_REFUSAL TURN=%s LOG=%s\n' "$TURN" "$C3LOG"; return 2; fi
    if [ "$REPORT_EXPOSED" -eq 1 ]; then printf 'SIGMA_V4C3_REFLECTION_CYCLE_COMPLETE TURN=%s STATUS=%s PLAN=%s\n' "$TURN" "$(cat "$C3_STATUS")" "$(cat "$C3_PLAN")"; fi
}
...
C3_CYCLE_TEXT=$(cat "$C3_CYCLE"); C3_PROGRESS_TEXT=$(cat "$C3_PROGRESS")
printf 'RESUME_PROFILE_COUNT=%s\nRESUME_COMPLETE_COUNT=%s\nRESUME_HOLD_COUNT=%s\n' "$START_PROFILE_COUNT" "$START_COMPLETE_COUNT" "$START_HOLD_COUNT"
...
if [ ! -s "$E/SIGMA_V4C3R1_INITIALIZED.memory" ]; then
    START_C3_LOG="$LOG/startup.c3.log"; run_vm C3_BASELINE "$C3_BC" "$START_C3_LOG" || exit 47; "$P/bin/cat" "$START_C3_LOG"
fi

TURN=0
while :; do
    TS=$("$P/bin/date" -u +%Y%m%dT%H%M%SZ)
    MLOG="$LOG/$TS.$TURN.manager.log"; ALOG="$LOG/$TS.$TURN.arbiter.log"; BLOG="$LOG/$TS.$TURN.learner.log"; C3LOG="$LOG/$TS.$TURN.reflect.log"
    run_vm MANAGER "$C2_BC" "$MLOG" || exit 50
    if "$P/bin/grep" -F 'STATUS REFUSE_' "$MLOG" >/dev/null 2>&1; then printf 'HOLD=NATIVE_MANAGER_REFUSAL TURN=%s LOG=%s\n' "$TURN" "$MLOG"; exit 51; fi
    if "$P/bin/grep" -F 'DOCUMENT_HELD 1' "$MLOG" >/dev/null 2>&1; then printf 'HOLD=NATIVE_DOCUMENT_HOLD TURN=%s LOG=%s\n' "$TURN" "$MLOG"; exit 52; fi
    run_vm ARBITER "$A3_BC" "$ALOG" || exit 53
    NATIVE_ACTION=$(cat "$ACTION"); NATIVE_TARGET=$(cat "$TARGET")
    ...
    case "$NATIVE_ACTION" in
        ...
        LEARN_RECEIVED_CONTEXT|RESUME_RETRYABLE_CONTEXT)
            ACTIVE_CONTEXT=$(cat "$B4_CONTEXT"); [ "$NATIVE_TARGET" = "$ACTIVE_CONTEXT" ] || { printf 'HOLD=NATIVE_TARGET_CONTEXT_MISMATCH TURN=%s ACTION=%s TARGET=%s ACTIVE=%s\n' "$TURN" "$NATIVE_ACTION" "$NATIVE_TARGET" "$ACTIVE_CONTEXT"; exit 55; }
            run_vm LEARNER "$B4_BC" "$BLOG" || exit 56 ;;
        WAIT_NO_ELIGIBLE_WORK) "$P/bin/sleep" "$IDLE_SLEEP_SECONDS" ;;
        *) printf 'HOLD=UNSUPPORTED_NATIVE_ACTION TURN=%s ACTION=%s TARGET=%s\n' "$TURN" "$NATIVE_ACTION" "$NATIVE_TARGET"; exit 57 ;;
    esac
    ...
    if [ $((TURN % HEALTH_CHECK_TURNS)) -eq 0 ]; then
        ...
        PHASE=$(cat "$E/SIGMA_V4C2R2_PHASE.memory"); ACTIVE_DOC=$(cat "$E/SIGMA_V4C2R2_ACTIVE_DOC.memory"); STATUS=$(cat "$MANAGER_STATUS")
        C3_CYCLE_TEXT=$(cat "$C3_CYCLE"); C3_PROGRESS_TEXT=$(cat "$C3_PROGRESS")
