...
SRC="$E/SIGMA_DOCUMENT_SURVEY_V2_5B.sigma"
BC="$E/SIGMA_DOCUMENT_SURVEY_V2_5B.sigmab"
EXPECTED_SOURCE=9e49ef9ca44f63a0174ac4a08b467544449adba79cf09af356397cd0d25b6072
...
SURVEY_MEMORY="$E/SIGMA_V25B_DOCUMENT_SURVEY.memory"
...
printf 'SIGMA_PHASE=V25B_FULL_CORPUS_DOCUMENT_SURVEY\n'
...
while [ "$CYCLE" -le "$MAX_CYCLES" ]; do
    ...
    if [ "$RC" -ne 0 ]; then
        printf 'V25B_FULL_CORPUS_SURVEY=FAIL\n'
...
if [ "$COMPLETE" -eq 1 ] && [ "$FINAL_COMMITTED" -eq "$SNAPSHOT_COUNT" ]; then
    printf 'V25B_FULL_CORPUS_SURVEY=PASS\n'
    printf 'NEXT_ACTION=BUILD_V26_BOUNDED_SEGMENT_CURSOR_PREFLIGHT\n'
    exit 0
fi

printf 'V25B_FULL_CORPUS_SURVEY=INCOMPLETE\n'
printf 'NEXT_ACTION=RERUN_SAME_RUNNER_TO_RESUME_PERSISTED_SURVEY\n'
exit 51
