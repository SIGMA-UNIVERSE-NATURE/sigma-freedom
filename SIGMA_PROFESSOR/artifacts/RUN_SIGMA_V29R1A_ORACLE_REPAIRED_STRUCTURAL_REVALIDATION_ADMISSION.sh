...
printf 'SIGMA_PHASE=V29R1A_ORACLE_REPAIRED_STRUCTURAL_REVALIDATION_ADMISSION\n'
...
"$P/bin/grep" -F 'BASELINE_FOUND 1' "$LOG/REAL_REVALIDATION.log" >/dev/null || exit 84
"$P/bin/grep" -F 'COMMITTED_DEEP_SEGMENT_COUNT 2' "$LOG/REAL_REVALIDATION.log" >/dev/null || exit 85
"$P/bin/grep" -F 'DISTINCT_DEEP_ANCHOR_COUNT 2' "$LOG/REAL_REVALIDATION.log" >/dev/null || exit 86
"$P/bin/grep" -F 'REVALIDATION_READY 1' "$LOG/REAL_REVALIDATION.log" >/dev/null || exit 87
"$P/bin/grep" -F 'REVALIDATION_APPEND_RC 0' "$LOG/REAL_REVALIDATION.log" >/dev/null || exit 88

REAL_BASELINE=$("$P/bin/grep" '^BASELINE_ANCHOR ' "$LOG/REAL_REVALIDATION.log" | "$P/bin/head" -n1 | "$P/bin/sed" 's/^BASELINE_ANCHOR //')
REAL_MATCHING_COUNT=$("$P/bin/grep" '^MATCHING_BASELINE_SEGMENT_COUNT ' "$LOG/REAL_REVALIDATION.log" | "$P/bin/head" -n1 | "$P/bin/awk" '{print $2}')
REAL_RESULT=$("$P/bin/grep" '^REVALIDATION_RESULT ' "$LOG/REAL_REVALIDATION.log" | "$P/bin/head" -n1 | "$P/bin/awk" '{print $2}')

[ -n "$REAL_BASELINE" ] || exit 89
case "$REAL_RESULT" in
    REOBSERVED|NOT_REOBSERVED) ;;
    *) exit 90 ;;
esac

printf 'REAL_NATIVE_BASELINE_ANCHOR=%s\n' "$REAL_BASELINE"
printf 'REAL_NATIVE_MATCHING_BASELINE_SEGMENT_COUNT=%s\n' "$REAL_MATCHING_COUNT"
printf 'REAL_NATIVE_REVALIDATION_RESULT=%s\n' "$REAL_RESULT"
...
"$P/bin/grep" -F "REVALIDATION_RESULT $REAL_RESULT" "$LOG/REAL_REVALIDATION_FRESH_VM_REUSE.log" >/dev/null || exit 93
"$P/bin/grep" -F "BASELINE_ANCHOR $REAL_BASELINE" "$LOG/REAL_REVALIDATION_FRESH_VM_REUSE.log" >/dev/null || exit 94
REUSE_SHA=$("$P/bin/sha256sum" "$V29_STATE" | "$P/bin/awk" '{print $1}')
[ "$REUSE_SHA" = "$REAL_REVALIDATION_SHA" ] || exit 95

: > "$V29_STATE"
run_v29 REAL_REVALIDATION_REPLAY
RC=$?
[ "$RC" -eq 0 ] || exit 96
"$P/bin/grep" -F "REVALIDATION_RESULT $REAL_RESULT" "$LOG/REAL_REVALIDATION_REPLAY.log" >/dev/null || exit 97
"$P/bin/grep" -F "BASELINE_ANCHOR $REAL_BASELINE" "$LOG/REAL_REVALIDATION_REPLAY.log" >/dev/null || exit 98
REPLAY_SHA=$("$P/bin/sha256sum" "$V29_STATE" | "$P/bin/awk" '{print $1}')
printf 'REAL_REVALIDATION_REPLAY_SHA256=%s\n' "$REPLAY_SHA"
[ "$REPLAY_SHA" = "$REAL_REVALIDATION_SHA" ] || exit 99

"$P/bin/printf" 'one two\nthree four' > "$SYNTH_DIR/Q.document"
"$P/bin/cat" > "$SYNTH_SURVEY" <<'EOF_SYN_POS_SURVEY'
DOC=Q || SURVEY_STATUS=COMPLETE || BEST_LOCAL_RELATION=alpha => beta || COMMIT=YES
EOF_SYN_POS_SURVEY
"$P/bin/cat" > "$SYNTH_EVIDENCE" <<'EOF_SYN_POS_EVIDENCE'
WORK=Q || CURSOR= || BEST_LOCAL_RELATION=alpha => beta || COMMIT=YES
EOF_SYN_POS_EVIDENCE
...
run_v29 SYNTHETIC_REOBSERVED_POSITIVE
RC=$?
[ "$RC" -eq 0 ] || exit 100
"$P/bin/grep" -F 'DEEP_RELEARN_COMPLETE 1' "$LOG/SYNTHETIC_REOBSERVED_POSITIVE.log" >/dev/null || exit 101
"$P/bin/grep" -F 'MATCHING_BASELINE_SEGMENT_COUNT 1' "$LOG/SYNTHETIC_REOBSERVED_POSITIVE.log" >/dev/null || exit 102
"$P/bin/grep" -F 'REVALIDATION_RESULT REOBSERVED' "$LOG/SYNTHETIC_REOBSERVED_POSITIVE.log" >/dev/null || exit 103
...
printf 'REAL_STRUCTURAL_REVALIDATION_RESULT_ACCEPTED_AS_NATIVE_EVIDENCE=PASS\n'
...
printf 'SYNTHETIC_REOBSERVED_BRANCH=PASS\nNEGATIVE_NOT_REOBSERVED=PASS\n'
...
printf 'NEXT_ACTION=CHECKPOINT_V29R1A_THEN_BUILD_REVALIDATION_TO_REVISIT_OR_ARCHIVE_FOR_NOW_PREFLIGHT\n'
