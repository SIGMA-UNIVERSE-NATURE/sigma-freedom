...
printf '%s\n' '=== T10A RECONSTRUCTION V1 ===' "SIGMAC_SHA256=$(sha "$SIGMAC")" "VM_SHA256=$(sha "$VM")" "SOURCE_SHA256=$(sha "$SRC")" "HOST_MECHANICAL_ABI=$ABI" "FORBIDDEN_SEMANTIC_ORACLE_TOKEN_COUNT_IN_SOURCE=$FC" 'HISTORICAL_T10A_BOUNDS_IMPORTED=NO'
...
"$SIGMAC" "$SRC" "$BC" >"$EXEC/compile.stdout" 2>"$EXEC/compile.stderr"; CRC=$?; echo "COMPILE_RC=$CRC"; cat "$EXEC/compile.stdout"; cat "$EXEC/compile.stderr"; [ "$CRC" -eq 0 ]&&[ -f "$BC" ]||fail COMPILE_FAILED 9
SF="$(sha "$SRC")"; BF="$(sha "$BC")"; echo "SOURCE_SHA256_AFTER_COMPILE=$SF"; echo "BYTECODE_SHA256_AFTER_COMPILE=$BF"; mkdir -p "$IN" "$OUT" "$CASES"
N=0; P=0; F=0; NZ=0; SL=0; SENT=0; LEAK=0; CFA=''; CFB=''; RIA=''; RSA=''
...
run(){
 ...
 echo "CASE_${I}_LABEL=$L"; echo "CASE_${I}_INPUT_SHA256=$IS"; echo "CASE_${I}_VM_RC=$RC"; echo "CASE_${I}_VM_STDOUT_SHA256=$OS"; echo "--- RAW SIGMA VM CASE $I STDOUT ---"; cat "$O"; echo "--- RAW SIGMA VM CASE $I STDERR ---"; cat "$ER"; echo "CASE_${I}_POST_VM_TEST_ORACLE_STARTED=YES"
 ...
 [ "$AL" = YES ]&&P=$((P+1))||F=$((F+1)); echo "CASE_${I}_POST_VM_ALIGNMENT=$AL"; [ "$L" = CFA ]&&CFA="$acc"; [ "$L" = CFB ]&&CFB="$acc"; [ "$L" = REPLAY_A ]&&{ RIA="$IS"; RSA="$OS"; }; if [ "$L" = REPLAY_B ];then [ "$IS" = "$RIA" ]&&[ "$OS" = "$RSA" ]||F=$((F+1));fi
}
